...
class BusSeatMissionScene: SKScene, UIGestureRecognizerDelegate,SKPhysicsContactDelegate {
   ...
   let CategoryRightHand : UInt32 = 0x1 << 4
   let CategoryWall : UInt32 = 0x1 << 5
   //노트 충돌 비트마스크 지정.
...
extension BusSeatMissionScene {
   ...
   func framewall(){
       ...
       wall.physicsBody?.categoryBitMask = CategoryWall
   ...
   func drawChildRightHand() {
       ...
       childRightHand.physicsBody?.categoryBitMask = CategoryRightHand
       childRightHand.physicsBody?.contactTestBitMask = CategoryWall
